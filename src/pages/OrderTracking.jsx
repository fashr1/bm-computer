    ...
    <Shell>
      <div className="grid items-start gap-6 lg:grid-cols-[1fr_360px]">
        <section className="rounded-2xl border border-line bg-surface p-6">
          ...
          {/* เลขพัสดุ/ขนส่ง เมื่อเริ่มจัดส่ง */}
          {order.tracking_no && (
          ...
          {canCancel && (
            <button onClick={() => setCancelOpen(true)} className="mt-4 inline-flex items-center gap-1.5 rounded-lg border border-line px-4 py-2 text-sm font-semibold text-brand-600 transition-colors hover:bg-brand-50 dark:hover:bg-brand-600/10 cursor-pointer">
              <Icon name="x" size={15} /> {order.status === 'pending' ? t('orders.cancelBtn') : t('orders.cancelReqBtn')}
            </button>
          )}
        </section>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-[420px] rounded-2xl border border-line bg-surface p-6 shadow-2xl">
        ...
        <div className="mt-4 flex gap-2">
          <button disabled={busy} onClick={submit} className="flex-1 rounded-xl bg-brand-600 py-2.5 font-semibold text-white hover:bg-brand-700 disabled:opacity-60 cursor-pointer">{busy ? t('common.loading') : t('orders.cancelConfirm')}</button>
          <button disabled={busy} onClick={onClose} className="flex-1 rounded-xl border border-line py-2.5 font-semibold hover:bg-surface2 cursor-pointer">{t('orders.cancelKeep')}</button>
        </div>
      </div>
    </div>
  )
